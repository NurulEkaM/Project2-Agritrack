...
<div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden" id="riwayat-section">
    <div class="p-6 border-b border-gray-50 flex flex-col gap-4">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div class="flex items-center gap-4">
                <h3 class="text-lg font-bold text-gray-800">Riwayat Penjualan</h3>
                  ...
                  <select id="tableLimit" class="px-3 py-2 border border-gray-100 rounded-xl bg-[#F8FAFC] text-xs font-bold outline-none focus:border-green-500">
            ...
            <div class="flex flex-col md:flex-row gap-2 w-full md:w-auto">
               <button onclick="document.getElementById('modalPdf').classList.remove('hidden')" 
                    class="flex items-center gap-2 bg-red-600 text-white px-4 py-2 rounded-xl text-[10px] font-bold hover:bg-red-700 transition-all shadow-sm">
        ...
        <div id="modalPdf" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 hidden">
            <div class="bg-white p-6 rounded-3xl w-80 shadow-2xl">
                <h3 class="font-bold mb-4">Filter Laporan</h3>
                <form action="{{ route('transaksi.pdf') }}" method="GET">
                    <select name="bulan" class="w-full p-2 border rounded mb-2 text-sm">
                        @foreach(range(1, 12) as $m)
                            <option value="{{ $m }}" {{ now()->month == $m ? 'selected' : '' }}>{{ \Carbon\Carbon::create()->month($m)->translatedFormat('F') }}</option>
                        @endforeach
                    </select>
                    <select name="tahun" class="w-full p-2 border rounded mb-2 text-sm">
                        @foreach(range(2025, now()->year) as $y)
                            <option value="{{ $y }}">{{ $y }}</option>
                        @endforeach
                    </select>
                    <select name="jenis_pesanan" class="w-full p-2 border rounded mb-4 text-sm">
                        <option value="semua">Semua Jenis</option>
                        <option value="wa">WhatsApp</option>
                        <option value="datang_langsung">Datang Langsung</option>
                        <option value="online_shop">Online Shop</option>
                    </select>
                    <button type="submit" class="w-full bg-green-800 text-white py-2 rounded-xl font-bold text-sm">Cetak</button>
                    <button type="button" onclick="document.getElementById('modalPdf').classList.add('hidden')" class="w-full mt-2 text-xs font-bold text-gray-400">Batal</button>
                </form>
            </div>
        </div>

        <div class="flex border-b border-gray-100 gap-2 overflow-x-auto text-xs font-bold">
            <button type="button" onclick="switchTab('all')" id="tab-all" class="tab-btn py-2 px-4 text-green-700 border-b-2 border-[#064E3B] whitespace-nowrap">Semua Data ({{ $riwayat->count() }})</button>
            <button type="button" onclick="switchTab('datang_langsung')" id="tab-datang_langsung" class="tab-btn py-2 px-4 text-gray-400 hover:text-gray-600 border-b-2 border-transparent whitespace-nowrap"><i class="fas fa-store text-gray-400 mr-1"></i> Datang Langsung ({{ $riwayat->where('jenis_pesanan', 'datang_langsung')->count() }})</button>
            <button type="button" onclick="switchTab('wa')" id="tab-wa" class="tab-btn py-2 px-4 text-gray-400 hover:text-gray-600 border-b-2 border-transparent whitespace-nowrap"><i class="fab fa-whatsapp text-green-500 mr-1"></i> WhatsApp ({{ $riwayat->where('jenis_pesanan', 'wa')->count() }})</button>
            <button type="button" onclick="switchTab('online_shop')" id="tab-online_shop" class="tab-btn py-2 px-4 text-gray-400 hover:text-gray-600 border-b-2 border-transparent whitespace-nowrap"><i class="fas fa-shopping-bag text-blue-500 mr-1"></i> Online Shop ({{ $riwayat->where('jenis_pesanan', 'online_shop')->count() }})</button>
        </div>
    </div>
...
<script>
    ...
    const orderChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: ['#D9F99D', '#06B6D4', '#3B82F6'],
                borderWidth: 0,
                offset: [10, 10, 10]
            }]
        },
        options: {
            ...
            plugins: {
                legend: { position: 'bottom', labels: { boxWidth: 12, font: { size: 11, weight: 'bold' }, padding: 15 } },
                tooltip: { callbacks: { label: function(context) { let value = context.raw; let percentage = totalData > 0 ? ((value / totalData) * 100).toFixed(1) + '%' : '0%'; return ` ${context.label}: ${value} trx (${percentage})`; } } }
            }
        }
    });
    ...
    document.getElementById('add-item').addEventListener('click', function() {
        ...
        newRow.innerHTML = `
            ...
            <div class="md:col-span-2 flex justify-end">
                <button type="button" class="remove-item text-red-500 hover:text-red-700 text-xs font-bold uppercase tracking-widest bg-red-50 px-3 py-2 rounded-lg">Hapus</button>
            </div>
        `;
    ...
    document.addEventListener('click', function(e) { if (e.target && e.target.classList.contains('remove-item')) { e.target.closest('.item-row').remove(); } });

    let currentTab = 'all';
    function switchTab(type = currentTab) {
        currentTab = type;
        ...
        document.getElementById(`tab-${type}`).classList.remove('text-gray-400', 'border-transparent');
        document.getElementById(`tab-${type}`).classList.add('text-green-700', 'border-[#064E3B]');
        ...
        rows.forEach(row => {
            ...
            if (matchTab && matchSearch) {
                visibleCount++;
                if (limit === 'all' || renderedCount < parseInt(limit)) {
                    row.style.display = '';
                    renderedCount++;
                } else {
    ...
    document.getElementById('trxSearch').addEventListener('keyup', () => switchTab());
    document.getElementById('tableLimit').addEventListener('change', () => switchTab());
    switchTab();
</script>
@endsection